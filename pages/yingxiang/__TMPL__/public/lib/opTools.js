...
(function($, cornerstone, cornerstoneMath, cornerstoneTools) {
    ...
    function onImageRendered(e, eventData) {
        ...
        for (var i = 0; i < toolData.data.length; i++) {
            ...
            // Draw text
            if (area !== undefined) {
                areaText = '面积: ' + area.toFixed(2) + ' mm' + String.fromCharCode(178);
                textSize = context.measureText(areaText);
            } else {
                var stdDevText = '标准差: ' + meanStdDev.stdDev.toFixed(2);
            ...
            context.fillStyle = color;
            if (meanStdDev) {
                cornerstoneTools.drawTextBox(context, '平均值: ' + meanStdDev.mean.toFixed(2), textX, textY - fontHeight - 5, color);
                cornerstoneTools.drawTextBox(context, '标准差: ' + meanStdDev.stdDev.toFixed(2), textX, textY, color);
            }
...
(function($, cornerstone, cornerstoneTools) {
    ...
    function mouseDragCallback(e, eventData) {
        ...
        //broadcast
        emitMessage('Pan-Listener|' + eventData.viewport.translation.x + '|' + eventData.viewport.translation.y);
        return false; // false = cases jquery to preventDefault() and stopPropagation() this event
    }
...
(function($, cornerstone, cornerstoneMath, cornerstoneTools) {
    ...
    function onImageRendered(e, eventData) {
        ...
        for (var i = 0; i < toolData.data.length; i++) {
            ...
            var areaText = '面积: ' + area.toFixed(2) + ' mm^2';
...
(function($, cornerstone, cornerstoneTools) {
    ...
    function mouseDragCallback(e, eventData) {
        ...
        cornerstone.setViewport(eventData.element, eventData.viewport);
        //broadcast
        emitMessage('WCWL-Listener|' + eventData.viewport.voi.windowWidth + '|' + eventData.viewport.voi.windowCenter);
        return false; // false = cases jquery to preventDefault() and stopPropagation() this event
    }
    ...
    function touchDragCallback(e, eventData) {
        ...
        cornerstone.setViewport(dragData.element, dragData.viewport);
        //broadcast
        emitMessage('WCWL-Listener|' + dragData.viewport.voi.windowWidth + '|' + dragData.viewport.voi.windowCenter);
    }
...
(function($, cornerstone, cornerstoneTools) {
    ...
    function changeViewportScale(viewport, ticks) {
        ...
        //broadcast
        emitMessage('Zoom-Listener|' + viewport.scale + '|na');
