function setupViewportOverlays(element, data) {
    ...
    $(topLeft[0]).text(data.patient_name);
    $(topLeft[1]).text(data.patient_id);
    $(topRight[0]).text(data.description);
    $(topRight[1]).text(data.study_date);
    ...
    function onNewImage(e, eventData) {
        ...
        var pageId = stack.currentImageIdIndex + 1;
        $(bottomLeft[2]).text("Image: " + (stack.currentImageIdIndex + 1) + "/" + stack.imageIds.length);
        if (isEmitScrollImageAction) {
            emitMessage('Scroll|' + stack.currentImageIdIndex + '|na');
        }
        isEmitScrollImageAction = true;
    }
